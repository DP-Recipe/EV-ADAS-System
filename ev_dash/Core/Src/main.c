...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	  ...
	  if(htim->Instance==TIM3) sensor_flag=1;/* 10 ms EV tick   */
...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start(&htim2);/* TIM1: 10ms IRQ      */
	  ...
	  uint16_t adc0 = Read_ADC_Channel(ADC_CHANNEL_0);
	  uint16_t adc1 = Read_ADC_Channel(ADC_CHANNEL_1);
	  uint16_t adc2 = Read_ADC_Channel(ADC_CHANNEL_2);
	  uint16_t adc3 = Read_ADC_Channel(ADC_CHANNEL_3);
